...
class WorkspaceMemory(BaseMemory):
    """
    Manages workspace-specific context and configurations for AI features.
    """

    def __init__(self, prefix: str = "ai:workspace") -> None:
        self.prefix = prefix

    def _make_key(self, workspace_id: str) -> str:
    ...
    async def get_context(self, key: str, **kwargs: Any) -> list[dict[str, str]]:
        ...
        try:
            ...
            for k, v in data.items():
                context.append(
                    {"role": "system", "content": f"[Workspace Memory] {k}: {v}"}
                )
            return context
        except Exception:
            return []
    ...
    async def store(self, key: str, data: Any, **kwargs: Any) -> None:
        ...
        # Cache workspace configurations for 1 year
