...
class CodePlanner:
    def create_plan(
        ...
        try:
            messages = [
                {
                    "role": "system",
                    "content": "You are a Staff Technical Planner. Return plain text instructions.",
                },
                {"role": "user", "content": prompt},
            ]
            content = llm_manager.generate_sync(messages=messages, temperature=0.2)
    ...
    def update_plan_status(
        self, db: Session, workspace_id: UUID, plan_id: UUID, status: str
    ) -> CodePlan | None:
        plan = (
            db.query(CodePlan)
            .filter(CodePlan.id == plan_id, CodePlan.workspace_id == workspace_id)
            .first()
        )
        if plan:
            plan.status = status
