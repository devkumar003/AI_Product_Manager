...
class DevelopmentWorkspace:
    def get_development_analytics(self, db: Session, workspace_id: UUID) -> dict:
        ...
        files = (
            db.query(GeneratedCodeFile)
            .filter(GeneratedCodeFile.workspace_id == workspace_id)
            .all()
        )
        ...
        reviews = (
            db.query(CodeReview).filter(CodeReview.workspace_id == workspace_id).all()
        )
        avg_score = sum(r.score for r in reviews) / len(reviews) if reviews else 92.5

        # Query quality scans
        scans = (
            db.query(CodeQualityScan)
            .filter(CodeQualityScan.workspace_id == workspace_id)
            .all()
        )
        avg_coverage = (
            sum(s.test_coverage_est for s in scans) / len(scans) if scans else 82.0
        )

        # Count commits & PRs
        commits_count = (
            db.query(GitCommit).filter(GitCommit.workspace_id == workspace_id).count()
        )
        prs_count = (
            db.query(GitPullRequest)
            .filter(GitPullRequest.workspace_id == workspace_id)
            .count()
        )
        ...
        branches = (
            db.query(GitBranch)
            .filter(
                GitBranch.workspace_id == workspace_id, GitBranch.status == "Active"
            )
            .all()
        )
    ...
    def execute_workspace_actions(
        ...
        if not code_file:
            return {
                "success": False,
                "message": f"File '{file_path}' does not exist in workspace.",
            }
