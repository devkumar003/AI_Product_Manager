...
class MCPService:
    ...
    def register_mcp_server(
        ...
        db.refresh(server)
        logger.info(
            f"Registered MCP Server {name} ({url}) for workspace {workspace_id}"
        )
        return server
    ...
    def list_mcp_servers(self, db: Session, workspace_id: UUID) -> list[MCPServer]:
        return (
            db.query(MCPServer)
            .filter(
                MCPServer.workspace_id == workspace_id, MCPServer.deleted_at.is_(None)
            )
            .all()
        )

    def delete_mcp_server(
        self, db: Session, workspace_id: UUID, server_id: UUID
    ) -> bool:
        server = (
            db.query(MCPServer)
            .filter(MCPServer.id == server_id, MCPServer.workspace_id == workspace_id)
            .first()
        )
        if not server:
            return False
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                ...
                response = await client.get(
                    f"{server.url}/tools", headers=server.headers or {}
                )
            ...
            logger.warn(
                f"Failed to connect to MCP Server {server.name} at {server.url}, falling back to default tools: {e}"
            )
    ...
    async def execute_tool(
        ...
        try:
            ...
            logger.error(
                f"Failed to execute tool {tool_name} on MCP Server {server.name}: {e}"
            )
