...
from app.models.integration import IntegrationWebhook, IntegrationLog
...
class WebhookEngine:
    def register_webhook(
        self, db: Session, workspace_id: UUID, name: str, target_url: str, events: List[str], secret_token: str | None = None
    ) -> IntegrationWebhook:
        ...
        webhook = IntegrationWebhook(
            ...
            events=events,
            secret_token=secret_token or "whsec_" + hashlib.sha256(name.encode()).hexdigest()[:16],
            is_active=True
        )
        db.add(webhook)
        db.commit()
        db.refresh(webhook)
        logger.info(f"Registered Webhook {name} targeting {target_url} for workspace {workspace_id}")
        return webhook

    def list_webhooks(self, db: Session, workspace_id: UUID) -> List[IntegrationWebhook]:
    ...
    async def trigger_event(self, db: Session, workspace_id: UUID, event: str, payload: Dict[str, Any]) -> int:
        """Dispatches an event payload to all webhooks subscribed to it."""
        subscribers = db.query(IntegrationWebhook).filter(
            IntegrationWebhook.workspace_id == workspace_id,
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            for sub in subscribers:
                ...
                if event in sub.events or "*" in sub.events:
                    ...
                    error_msg = None
                    try:
                        res = await client.post(sub.target_url, json=payload, headers=headers)
                    ...
                    log = IntegrationLog(
                        ...
                        status=status,
                        payload={"event": event, "target_url": sub.target_url, "payload": payload},
        ...
        db.commit()
        logger.info(f"Dispatched event {event} to {dispatched_count} webhook subscribers.")
