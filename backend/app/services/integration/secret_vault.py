...
class SecretVault:
    ...
    def store_secret(self, db: Session, workspace_id: UUID, key: str, value: str) -> EncryptedSecret:
    ...
    def get_secret(self, db: Session, workspace_id: UUID, key: str) -> str | None:
        ...
        try:
            ...
            logger.error(f"Failed to decrypt secret {key} for workspace {workspace_id}: {e}")
            return None
