    ...
    async def execute(
        ...
        for step in self.steps:
            ...
            if step.condition and not step.condition(context):
                logger.info(
                    f"Skipping conditional step '{step.engine_name}' in chain '{self.name}'"
                )
                continue
...
def build_idea_validation_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "idea": ctx["idea"],
                "additional_context": ctx.get("context", ""),
            },
            output_key="analysis",
        )
    )
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "idea": ctx["idea"],
                "analysis_summary": _json_dump(ctx.get("analysis", {})),
            },
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "idea": ctx["idea"],
                "validation_summary": _json_dump(ctx.get("validation", {})),
            },
...
def build_prd_generation_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "idea": ctx["idea"],
                "validation_summary": _json_dump(ctx.get("analysis", {})),
            },
            output_key="discovery",
        )
    )
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "product_discovery": _json_dump(ctx.get("discovery", {})),
                "idea": ctx["idea"],
            },
...
def build_architecture_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            engine_name="requirement_generator",
            input_builder=lambda ctx: {
                "product_discovery": ctx.get("idea", ""),
                "idea": ctx.get("idea", ""),
            },
            output_key="requirements",
        )
...
def build_roadmap_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "features": ctx.get("features", ctx.get("idea", "")),
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "features": _json_dump(ctx.get("prioritization", {})),
                "timeline_months": ctx.get("timeline_months", 12),
            },
            output_key="roadmap",
        )
    )
...
def build_sprint_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "roadmap": ctx.get("roadmap_data", ctx.get("idea", "")),
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "sprint_data": _json_dump(ctx.get("sprint", {}))
            },
            output_key="tasks",
        )
    )
...
def build_risk_assessment_workflow(executor: EngineExecutor) -> PromptChain:
    ...
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "project_context": ctx.get("project_context", ctx.get("idea", ""))
            },
            output_key="risks",
        )
    )
    chain.add_step(
        ChainStep(
            ...
            input_builder=lambda ctx: {
                "task_breakdown": _json_dump(ctx.get("risks", {}))
            },
            output_key="costs",
        )
    )
